...
try:
    ...
    raise ModuleNotFoundError(
        "Plotly is required for PureLineCharts. "
        "Install it with `pip install plotly` to enable spectral chart logging."
    ) from exc


class PureLineCharts(Callback):
    """Compare one predicted pure spectrum against up to two original pure spectra each epoch."""
    ...
    def on_validation_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        for material in materials:
            ...
            predicted = sample_fn(n=1, conditions=condition, device=device)
            pred_line = predicted.view(1, -1).detach().cpu().numpy()[0]
            pred_line = np.clip((pred_line + 1.0) / 2.0, 0.0, 1.0)
    ...
    def _write_chart(
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=pred_line.tolist(),
                mode="lines",
                name=f"Predicted pure {material} (epoch {epoch})",
                line={"width": 3},
            )
        )
        ...
        fig.update_layout(
            title=f"Pure {material.upper()} comparison - Epoch {epoch}",
