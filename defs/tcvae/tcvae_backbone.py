...
class GatedFiLM(nn.Module):
    ...
    def __init__(self, cond_dim: int, d_model: int, gate_init: float = 0.1) -> None:
        ...
        init_gate = torch.tensor(float(gate_init), dtype=torch.float32).clamp(1e-6, 1.0 - 1.0e-6)
...
class DualPathDecoder(nn.Module):
    ...
    def __init__(
        ...
        init_weight = torch.tensor(raw_weight, dtype=torch.float32).clamp(1e-6, 1.0 - 1.0e-6)
