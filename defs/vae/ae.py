...
class FiLM(nn.Module):
    """
    A film layer, to embed the abundances on spectra. Classic Feature wIse Linear Modulation.

    Args:
        condition_dim (int): The dim of the condition embedding.
        feature_dim (int): The dim of our spectra.

    Logic:
        gamma_proj = Linear(cond, feat)
        beta_proj = Linear(cond, feat)
        gamma = gamma_proj(condition); beta = beta_proj(conditon)
        spectrum = spectrum * (1 + gamma) + beta
    """
    ...
    def forward(self, x:(torch.Tensor), c:(torch.Tensor)):
        ...
        Args: 
            x (torch.Tensor): The data tensor, of shape (B, feature_dim)
            c (torch.Tensor): The condition tensor, of shape (B, condition_dim)
        """
...
class VariationalDecoder(nn.Module):
    ...
    """
    def __init__(self, n_bands:(int), out_ch:(int), latent_dim:(int), condition_dim:(int), seq_len_final:(int), conv_layers:(list[int]), kernel_size:(int), padding:(int), output_padding:(int), pool_stride:(int), mlp_layers:(list[int]), act_fn:(str)):
        super().__init__()
        # Basic init stuff
        self.n_bands = n_bands
        self.out_ch = out_ch
        ...
        # The initial projector that takes in the latent dim, projects it to the amount of mlp layers
        self.latent_proj = nn.Linear(self.latent_dim, mlp_layers[0])

        # FiLM
        self.film = FiLM(condition_dim=self.condition_dim, feature_dim=mlp_layers[0])

        # Build MLP layers
        mlp_list = [self.act_fn]
        for i in range(len(self.mlp_layers)-1):
            mlp_list.extend([
                nn.Linear(in_features=self.mlp_layers[i], out_features=self.mlp_layers[i+1]),
                self.act_fn
    ...
    def forward(self, z:(torch.Tensor), c:(torch.Tensor)):
        """
        Forward passes a latent vector z, with the condition c
        """
        ...
        if decoded.shape[-1] != self.n_bands: 
            decoded = torch.nn.functional.interpolate(decoded, size=self.n_bands, mode='linear', align_corners=False) 
...
class VariationalConditioner(nn.Module):
    ...
    def __init__(self, n_endmembers:(int), embedding_dim:(int), embedding_layers:(list[int]), act_fn:(str)='relu'):
        ...
        if len(embedding_layers) > 0:
            # Init layer
            ab_list.extend([nn.Linear(n_endmembers, embedding_layers[0]), self.act_fn])
            # Serialized hidden Layers
            for i in range(len(embedding_layers) - 1): ab_list.extend([nn.Linear(embedding_layers[i], embedding_layers[i+1]), self.act_fn])
            # Final Projection
            ab_list.append(nn.Linear(embedding_layers[-1], embedding_dim))
        else:
            # Direct mapping if no hidden layers
            ab_list.append(nn.Linear(n_endmembers, embedding_dim))
        # Compound everything
        self.embedding_layers = nn.Sequential(*ab_list)

    def forward(self, ab:(torch.Tensor)):
        """
        ...
        Returns:
            embedded (torch.Tensor): Embeds the abundances vector (B, 3) -> (B, embedding_dim)
        """
        return self.embedding_layers(ab) 
